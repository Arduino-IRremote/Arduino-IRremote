...
- Just remove the line `IRrecv IrReceiver(IR_RECEIVE_PIN);` and/or `IRsend IrSender;` in your program, and replace all occurrences of `IRrecv` or `irrecv` with `IrReceiver`. Replace all `IRsend` or `irsend` with `IrSender`.
