...
int  IRrecv::getRClevel (int *offset,  int *used,  int t1)
{
	...
	if (*offset >= irparams.rawlen)  return SPACE ;  // After end of recorded buffer, assume SPACE.
	width      = irparams.rawbuf[*offset];
...
bool  IRrecv::decodeRC6 ()
{
	...
	if (irparams.rawlen < MIN_RC6_SAMPLES)  return false ;

	// Initial mark
	if (!MATCH_MARK(irparams.rawbuf[offset++],  RC6_HDR_MARK))   return false ;
	if (!MATCH_SPACE(irparams.rawbuf[offset++], RC6_HDR_SPACE))  return false ;
