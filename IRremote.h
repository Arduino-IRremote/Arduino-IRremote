...
private:
	...
	static int getRClevel(decode_results *results, int *offset, int *used,
			int t1);

	static long decodeNEC(decode_results *results);
	static long decodeSony(decode_results *results);
	static long decodeSanyo(decode_results *results);
	static long decodeMitsubishi(decode_results *results);
	static long decodeRC5(decode_results *results);
	static long decodeRC6(decode_results *results);
	static long decodePanasonic(decode_results *results);
	...
	long (*_decode_function)(decode_results *);
}
;
...
public:
	...
	// private:
	void enableIROut(int khz);VIRTUAL
	void mark(int usec);VIRTUAL
	void space(int usec);
}
;
